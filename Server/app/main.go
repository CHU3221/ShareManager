...
func loadConfig() {
	...
	if err != nil {
		log.Println("config.json이 없어 기본값으로 생성합니다.")
		appConfig = Config{
			APIKey:         "API_KEY_1234567890",
			StorageBaseDir: "/sharedrive",
			Port:           "1234",
			PublicDomain:   "http://localhost:1234",
		}
	...
	if err != nil {
		log.Fatal("설정 파일 파싱 오류:", err)
	}
}

func initDB() {
	var err error
	db, err = sql.Open("sqlite3", "./data/share.db")
	if err != nil {
	...
	db.Exec("ALTER TABLE shares ADD COLUMN max_downloads INTEGER DEFAULT 0;")
	db.Exec("ALTER TABLE shares ADD COLUMN current_downloads INTEGER DEFAULT 0;")
}
...
func authMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("X-API-Key") != appConfig.APIKey {
...
func main() {
	logFile, err := os.OpenFile("/var/log/share-server.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if err == nil {
	...
	log.Printf("공유 서버가 %s 포트에서 시작되었습니다.\n", appConfig.Port)
